...
\documentclass[mathserif,usenames,dvipsnames,svgnames,table]{beamer}
...
\begin{frame}
    ...
    \begin{columns}%[h]
      ...
      %\begin{column}{6cm}
      %    \begin{center}
      %        \includegraphics[scale=0.5]{solidx_logo}
      %    \end{center}
      %\end{column}
    \end{columns}
\end{frame}
...
%===============================================================================
\begin{frame}{}
    Braidpool is a (proposal for) a {\it \color{ForestGreen} decentralized
...
\begin{frame}{Simulations}
    \includegraphics[scale=0.35]{T_C_x.png}
\end{frame}
%===============================================================================
\begin{frame}{Outline}
    \begin{itemize}
        \item Shares and Weak Blocks
            \begin{itemize}
...
\begin{frame}{Shares and Weak Blocks}
    \begin{itemize}
        \item Braidpool accounts for mining "Shares" (also called ``Weak Blocks'')

        \item A "share" does not meet bitcoin's difficulty target $t_b$ but
            meets some lesser difficulty target $t$. Shares are a bitcoin block
            header which commits to additional data:
    \end{itemize}

    \includegraphics[scale=0.34]{bead\_fields.png}
%    \scriptsize
%    \begin{block}{Bead Fields}
%\begin{semiverbatim}
%    Version|Previous Block Hash|Merkle Root|Timestamp|Target|Nonce
%
...
%        \item Latency is zero {\only<2->{\color{red}(scaling)}}
...
%\vspace{0.3cm}
%{\onslide<2->{\it
%``These assumptions ultimately prove false, resulting either in the failure of the
%system, a substantial reduction in system scope, or in large, unplanned expenses
%required to redesign the system to meet its original goals.''}}
\end{frame}
%===============================================================================
\begin{frame}{Share Commitments}
    \begin{itemize}
        \item A share must commit to the additional data required by Braidpool
    ...
    \scriptsize
    \begin{semiverbatim}
    OutPoint(Value:0, scriptPubKey OP\_RETURN "BP"+<Braidpool Commitment>)
    OutPoint(Value:<block reward>, scriptPubKey <P2TR pool\_pubkey>)
    \end{semiverbatim}
    \normalsize
    ...
    {\tt Braidpool Commitment} = hash({\tt Braidpool Metadata})
    \begin{itemize}
        \item This prevents third parties from modifying consensus-critical data
            in a share
    \end{itemize}
\end{frame}
%===============================================================================
\begin{frame}{Share Metadata}
    \begin{columns}
        \column{\dimexpr\paperwidth-10pt}
        \includegraphics[scale=0.34]{metadata\_table.png}
    \end{columns}
    \begin{itemize}
        \item The additional timestamps provide required higher resolution bead
            timing information
    \end{itemize}
\end{frame}
%===============================================================================
\begin{frame}{Share Value}
    \begin{itemize}
        \item Braidpool will use the {\it \color{ForestGreen} Full Proportional}
            payout method (all rewards and fees distributed proportionally to
            hashers)
        \item Braidpool shares have an internal value which is equal to the
            work $w$ -- the estimate of the number of sha256d hashes performed
            (TeraHash Coin), weighted by the probability that the DAG generated
            an orphan:
    \end{itemize}


$$
P_{\ge 2}(T_C) = \sum_{k=2}^\infty \frac{\lambda(T_C)^k e^{-\lambda(T_C)}}{k!} = 1 - e^{-\lambda(T_C)} (1+\lambda(T_C))
\simeq \frac{\lambda^2(T_C)}{2}
$$

$$
\lambda(T_C) = \frac{T_C}{\rm block\ time} \left(\frac{\rm pool\ hashrate}{\rm total
\ hashrate}\right)
$$

$$
{\rm (share)} \qquad \qquad w = \frac{1}{x (1-P_{\ge 2})}
$$


\end{frame}
%===============================================================================
\begin{frame}{Share Tally}
    \begin{itemize}
        \item Shares are a statistical estimate of {\it work} (number of sha256d
            calculations performed)
        \item The BTC value of shares is only known at difficulty adjustment
            boundaries (every 2016 blocks $\sim$ 2 weeks)
        \item Shares can only be paid out {\it after} a difficulty adjustment
            window closes
        \item Shares can be sent to a new address (to effect instant payout via
            atomic swap or exchange trade)
    \end{itemize}
\end{frame}
%===============================================================================
\section{Consensus}
...
\begin{frame}{The Cohort}
    ...
    {\it Cohorts} define a {\it total order} such that {\it all} beads are ancestors
    of the next cohort, and descendants of the previous cohort:
    \begin{center}
        \includegraphics[scale=0.4]{thinbraid.png}
    \end{center}
    \vspace{0.1cm}
    \begin{center}
        \includegraphics[scale=0.4]{thickbraid.png}
    \end{center}
    Cohorts give us a way to measure the latency of the network WRT the bead
    rate and are used in the retarget algorithm, and in optimizing tx conflict
    selection.
%    Major contribution of this work: the algorithm to find cohorts.
%    \begin{block}{}
%        \begin{center}
%            \it A cohort is the analog of a block
%        \end{center}
%    \end{block}
\end{frame}
%===============================================================================
...
%\end{frame}
%===============================================================================
%\subsection{Miner-Selected Difficulty}
\begin{frame}{Miner-Selected Difficulty}
    \begin{itemize}
        ...
        \item Braidpool will choose your difficulty such that all miners have
            the same {\it variance}.
        \item Miners too small to achieve constant variance will be grouped into
            a {\it sub-pool}, which:
            \begin{itemize}
                \item has UHPO outputs in the parent pool as the
                    outputs managed by the sub-pool
...
\end{frame}
%===============================================================================
\begin{frame}{Derivatives and Instant Payout}
    \begin{itemize}
...
\begin{frame}{General Timeline}
    \begin{itemize}
        \item Braidpool V1
            \begin{itemize}
                \item Peer-to-Peer Layer (Kulpreet Singh)
                \item Braid Consensus (Bob McElrath)
                \item Update/UHPO signing (Bob McElrath, Jesse Posner's FROST code from
                    libsecp256k-zkp)
            \end{itemize}
        \item Braidpool V2
            \begin{itemize}
...
\begin{frame}{Summary/Conclusions}
    \begin{itemize}
        \item All the elements are present to construct a truly trustless decentralized
            mining pool (Shares, Braid consensus, large multi-sigs)
        \item The advent of Schnorr, FROST, ROAST, and MuSig2 were the missing piece
        \item This UHPO proposal should be contrasted with the (semi-trusted)
            Hub model originally proposed by Chris Belcher
        \item Goal will be to get existing pools to run atop Braidpool, and
            integrate with Stratum V2
    \end{itemize}
\end{frame}
%===============================================================================
